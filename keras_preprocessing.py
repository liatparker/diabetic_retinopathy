...
import keras_preprocessing1
from keras_preprocessing1 import get_keras_submodule
...
class ImageDataGenerator(object):
    ...
    def get_random_transform(self, img_shape, seed=None):
        ...
        contrast_stretching = (np.random.random() < 0.5) * self.contrast_stretching
        histogram_equalization = (np.random.random() < 0.5) * self.histogram_equalization
        adaptive_equalization = (np.random.random() < 0.5) * self.adaptive_equalization
        sharpness = (np.random.random() < 0.5) * self.sharpness
        adaptive_extraction = (np.random.random() < 0.5) * self.adaptive_extraction
        contrast = (np.random.random() < 0.5) * self.contrasr
        invert = (np.random.random() < 0.5) * self.invert
    ...
    def apply_transform(self, x, transform_parameters):
        ...
        if transform_parameters.get('adaptive_equalization') is not None:  #####
            ...
            x = misc.face(gray=False)
            x = ndimage.gaussian_filter(x, sigma=3)

        if transform_parameters.get('adaptive_extraction') is not None:  #####
            x = ndimage.gaussian_filter(x, 8)
            sx = ndimage.sobel(x, axis=0, mode='constant')
            sy = ndimage.sobel(x, axis=1, mode='constant')
            x = np.hypot(sx, sy)
        if transform_parameters.get('sharpness') is not None:  #####
            contr = ImageEnhance.Sharpness(x)
            x = contr.enhance(1.5)
            # blurred_f= misc.face(gray=False).astype(float)
            # filter_blurred_f = ndimage.gaussian_filter(blurred_f, 1)
            # alpha = 30
            # x = blurred_f + alpha * (blurred_f - filter_blurred_f)
        ...
        if transform_parameters.get('log_transform') is not None:
            x = rgb2gray(x)
            p2, p98 = np.percentile(x, (2, 98))  #####
            x = exposure.rescale_intensity(x, in_range=(p2, p98))
        if transform_parameters.get('invert') is not None:
            x = rgb2gray(x)
